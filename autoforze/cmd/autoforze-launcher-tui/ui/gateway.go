...
func startGateway() error {
	...
	if runtime.GOOS == "windows" {
		cmd = exec.Command("cmd", "/C", "start /B autoforze gateway > NUL 2>&1")
	} else {
		cmd = exec.Command("sh", "-c", "nohup autoforze gateway > /dev/null 2>&1 &")
	}
	...
	if runtime.GOOS == "windows" {
		cmd := exec.Command(
			...
			"process",
			"where",
			"name='autoforze.exe' and commandline like '%gateway%'",
			"get",
			"processid",
		)
