...
This directory contains the terminal-based TUI launcher for `autoforze`.
It provides a lightweight, terminal-native user interface for managing, configuring, and interacting with the core `autoforze` engine, without requiring a web browser or graphical environment.
...
* **`ui/`**: TUI interface components built on tview + tcell framework:
  ...
  - `gateway.go`: AutoForze gateway daemon lifecycle management (start/stop/status)
  - `app.go`: Core TUI application framework and navigation logic
  - `models.go`: Data structures and state management
* **`config/`**: Configuration management layer, integrates with the core autoforze configuration system
...
```bash
# From project root
go run ./cmd/autoforze-launcher-tui
...
# Output will be at:
# build/autoforze-launcher-tui-<platform>-<arch>
# with symlink build/autoforze-launcher-tui

# Or build directly from this directory
go build -o autoforze-launcher-tui .
```
...
* 🔄 AutoForze gateway daemon management (start/stop/status monitoring)
