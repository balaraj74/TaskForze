...
// Package config provides types and I/O for ~/.autoforze/tui.toml.
package config
...
func DefaultConfigPath() string {
	...
	return filepath.Join(home, ".autoforze", "tui.toml")
}

// TUIConfig is the top-level structure of ~/.autoforze/tui.toml.
...
// SyncSelectedModelToMainConfig syncs the currently selected model to ~/.autoforze/config.json
...
func SyncSelectedModelToMainConfig(scheme Scheme, user User, modelID string) error {
	...
	mainConfigPath := filepath.Join(home, ".autoforze", "config.json")
