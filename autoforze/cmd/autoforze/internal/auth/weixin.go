...
import (
	...
	"github.com/sipeed/autoforze/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the autoforze
config so you can start the gateway immediately.

Example:
  autoforze auth weixin`,
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your autoforze config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
