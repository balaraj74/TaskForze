...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: autoforze agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: autoforze auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: autoforze auth login --provider google-antigravity",
		)
	}
