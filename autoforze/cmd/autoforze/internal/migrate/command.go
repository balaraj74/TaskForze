...
import (
	...
	"github.com/sipeed/autoforze/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to autoforze",
		Args:  cobra.NoArgs,
		Example: `  autoforze migrate
  autoforze migrate --from openclaw
  autoforze migrate --dry-run
  autoforze migrate --refresh
  autoforze migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.autoforze)")
