...
`web/` contains the standalone WebUI launcher for AutoForze.
It is not just a frontend: it is a small launcher service that bundles a React dashboard, exposes a backend API, manages launcher authentication, and starts or attaches to the `autoforze gateway` process.

![AutoForze Launcher](./autoforze-launcher.png)
...
- A controlled way to start, stop, restart, and inspect the `autoforze gateway` subprocess.
...
At runtime the launcher and the main AutoForze engine are separate processes:
...
3. When allowed, it starts or attaches to `autoforze gateway -E`.
...
The launcher uses the same AutoForze config file as the main binary.

- Default app config path: `~/.autoforze/config.json`
- Override with environment variable: `AUTOFORZE_CONFIG`
- Override with a positional CLI argument: `autoforze-launcher /path/to/config.json`

Launcher-only settings are stored beside that app config:

- File name: `launcher-config.json`
- Default location: `~/.autoforze/launcher-config.json`
...
The launcher looks for the main AutoForze binary in this order:

1. `AUTOFORZE_BINARY`
2. A `autoforze` binary in the same directory as the launcher
3. `autoforze` from `PATH`

If onboarding or gateway startup cannot find the main binary, set `AUTOFORZE_BINARY` explicitly.

### Gateway Management

The launcher manages `autoforze gateway -E`.
...
- If `AUTOFORZE_LAUNCHER_TOKEN` is set, that token is used.
...
- GUI mode without stdout:
  ...
  - default log path: `~/.autoforze/logs/launcher.log`
  - if `AUTOFORZE_HOME` is set, use `$AUTOFORZE_HOME/logs/launcher.log`
  - env-pinned tokens are not reprinted there; the log only notes that `AUTOFORZE_LAUNCHER_TOKEN` is in use
...
1. Builds `../build/autoforze` for launcher development.
2. Starts the Go backend with `AUTOFORZE_BINARY` pointing at that binary.
...
4. Produces `build/autoforze-launcher`.

Override the output path if needed:

```bash
make build OUTPUT=/tmp/autoforze-launcher
```
...
```text
build/autoforze-launcher-<platform>-<arch>
```

and refreshes the `build/autoforze-launcher` symlink.
...
```bash
./build/autoforze-launcher
./build/autoforze-launcher -console
./build/autoforze-launcher -public
./build/autoforze-launcher -port 19999 /path/to/config.json
```
...
```bash
export AUTOFORZE_LAUNCHER_TOKEN="replace-with-a-long-random-token"
```
...
### The launcher cannot find `autoforze`

Set the main binary explicitly:

```bash
export AUTOFORZE_BINARY=/absolute/path/to/autoforze
```
...
- Official docs site: [docs.autoforze.io](https://docs.autoforze.io)
