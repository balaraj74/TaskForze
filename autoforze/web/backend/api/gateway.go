...
var gateway = struct {
	...
	logs                *LogBuffer
	pidData             *ppid.PidFileData // pid file data read from autoforze.pid.json
...
// isLikelyGatewayProcess returns whether PID appears to be a autoforze gateway
...
func isLikelyGatewayProcess(pid int) (bool, bool) {
	...
	if runtime.GOOS == "windows" {
		...
		if strings.HasPrefix(line, "\"") {
			if strings.Contains(line, "\"autoforze.exe\"") {
				return true, true
			}
			return false, false
		}
...
// represents "autoforze gateway ..." regardless of executable filename.
...
func (h *Handler) validateGatewayPidData(
	...
	if gatewayProcess, inspected := gatewayProcessMatcher(pidData.PID); inspected {
		if !gatewayProcess {
			return false, true, "pid process command is not autoforze gateway"
		}
		return true, true, ""
	}
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the autoforze executable
	execPath := utils.FindAutoforzeBinary()
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started autoforze gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the autoforze gateway subprocess.
//
