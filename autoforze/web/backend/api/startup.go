...
const (
	autoStartEntryName = "AutoForzeLauncher"
	launchAgentLabel   = "io.autoforze.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "autoforze-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=AutoForze Web",
			"Comment=Start AutoForze Web on login",
