...
import (
	...
	"github.com/sipeed/autoforze/pkg/auth"
	"github.com/sipeed/autoforze/pkg/config"
	ppid "github.com/sipeed/autoforze/pkg/pid"
	"github.com/sipeed/autoforze/web/backend/utils"
)
...
func startGatewayLikeProcess(t *testing.T) *exec.Cmd {
	...
	cmd = exec.Command("sh", "-c", "sleep 30 # autoforze gateway")
...
func writeTestPidFile(t *testing.T, data ppid.PidFileData) string {
	...
	path := filepath.Join(globalConfigDir(), ".autoforze.pid")
...
func resetGatewayTestState(t *testing.T) {
	...
	t.Setenv("AUTOFORZE_HOME", t.TempDir())
	t.Cleanup(func() {
...
func TestLooksLikeGatewayCommandLine(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "default autoforze gateway",
			cmdline: "/usr/local/bin/autoforze gateway -E",
			want:    true,
		},
		{
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-autoforze")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("AUTOFORZE_BINARY", invalidBinaryPath)
...
func TestFindAutoforzeBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "autoforze-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("AUTOFORZE_BINARY", mockBinary)

	got := utils.FindAutoforzeBinary()
	if got != mockBinary {
		t.Errorf("FindAutoforzeBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindAutoforzeBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When AUTOFORZE_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("AUTOFORZE_BINARY", "/nonexistent/autoforze-binary")

	got := utils.FindAutoforzeBinary()
	// Should not return the invalid path; falls back to "autoforze" or another found path
	if got == "/nonexistent/autoforze-binary" {
		t.Errorf("FindAutoforzeBinary() returned invalid env path %q, expected fallback", got)
	}
}
