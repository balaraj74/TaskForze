...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("AUTOFORZE_BUILTIN_SKILLS")
	if err := os.Setenv("AUTOFORZE_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(AUTOFORZE_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("AUTOFORZE_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("AUTOFORZE_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
