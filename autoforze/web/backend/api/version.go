...
var (
	...
	findAutoforzeBinaryForInfo   = resolveGatewayBinaryForVersionInfo
	runAutoforzeVersionOutput    = executeAutoforzeVersion
...
// resolveSystemVersionInfo prefers the actual autoforze binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(findAutoforzeBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runAutoforzeVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parseAutoforzeVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindAutoforzeBinary()
}
...
// executeAutoforzeVersion runs the version subcommand against the
// discovered autoforze executable.
func executeAutoforzeVersion(ctx context.Context, execPath string) (string, error) {
...
// parseAutoforzeVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parseAutoforzeVersionOutput(raw string) (systemVersionResponse, bool) {
