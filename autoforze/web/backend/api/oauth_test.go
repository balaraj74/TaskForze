...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("AUTOFORZE_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("AUTOFORZE_HOME", filepath.Join(tmp, ".autoforze")); err != nil {
		t.Fatalf("set AUTOFORZE_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("AUTOFORZE_HOME")
		} else {
			_ = os.Setenv("AUTOFORZE_HOME", oldPicoHome)
		}
	}
