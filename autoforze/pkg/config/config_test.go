  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "autoforze-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".autoforze", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithAutoforzeHome(t *testing.T) {
	t.Setenv("AUTOFORZE_HOME", "/custom/autoforze/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/autoforze/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with AUTOFORZE_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("AUTOFORZE_SSH_KEY_PATH", "")
...
// unchanged when AUTOFORZE_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", "")
	t.Setenv("AUTOFORZE_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when AUTOFORZE_KEY_PASSPHRASE
