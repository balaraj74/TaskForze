...
func TestSecurityPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "standard path",
			configDir: "/home/user/.autoforze/config.json",
			want:      "/home/user/.autoforze/.security.yml",
		},
		{
			name:      "nested path",
...
`
	...
	t.Run("test for env overwrite", func(t *testing.T) {
		...
		t.Setenv("AUTOFORZE_CHANNELS_QQ_APP_SECRET", "qq_app_secret_env")
		t.Setenv("AUTOFORZE_TOOLS_WEB_BRAVE_API_KEYS", "brave_key_env,abc")
		err2 := env.Parse(cfg2)
