...
```bash
chmod 600 ~/.autoforze/.security.yml
```
...
```bash
cp ~/.autoforze/config.json ~/.autoforze/config.json.backup
```

## Step 2: Create .security.yml
```bash
cp security.example.yml ~/.autoforze/.security.yml
```

## Step 3: Fill in your API keys
Edit ~/.autoforze/.security.yml and replace placeholders with your actual keys.

## Step 4: Simplify config.json (Recommended)
Remove sensitive fields from ~/.autoforze/config.json:
...
```bash
rm ~/.autoforze/config.json.backup
```
...
3. Set file permissions: chmod 600 ~/.autoforze/.security.yml
...
```bash
# Channels
export AUTOFORZE_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export AUTOFORZE_CHANNELS_DISCORD_TOKEN="discord-token-from-env"

# Web Tools
export AUTOFORZE_TOOLS_WEB_BRAVE_API_KEY="brave-key-from-env"
export AUTOFORZE_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"

# Skills
export AUTOFORZE_TOOLS_SKILLS_GITHUB_TOKEN="github-token-from-env"
```
...
- Verify the file permissions allow reading (chmod 600 ~/.autoforze/.security.yml)
