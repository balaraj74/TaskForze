...
type GatewayConfig struct {
	Host      string `json:"host"                env:"AUTOFORZE_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"AUTOFORZE_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"AUTOFORZE_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"AUTOFORZE_LOG_LEVEL"`
}
...
// The AUTOFORZE_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("AUTOFORZE_LOG_LEVEL"); envLevel != "" {
