...
func (c *OpenClawConfig) ConvertToAutoForze(sourceHome string) (*AutoForzeConfig, []string, error) {
	cfg := &AutoForzeConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via autoforze CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - AutoForze uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in AutoForze - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in AutoForze - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - AutoForze uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No AutoForze adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No AutoForze adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No AutoForze adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No AutoForze adapter available - consider iMessage instead",
		)
	}
...
func (c *AutoForzeConfig) ToStandardConfig() *config.Config {
