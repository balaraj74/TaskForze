...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.autoforze/workspace" {
		t.Errorf("expected workspace '~/.autoforze/workspace', got '%s'", workspace)
	}
...
func TestConvertToAutoForze(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToAutoForze("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.autoforze/workspace" {
		t.Errorf("expected workspace '~/.autoforze/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToAutoForzeWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToAutoForze("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToAutoForzeWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToAutoForzeWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.autoforze/workspace" {
		t.Errorf("expected workspace '~/.autoforze/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
