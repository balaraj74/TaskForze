...
import (
	...
	"github.com/sipeed/autoforze/pkg/migrate/internal"
)

// OpenclawHomeEnvVar is the environment variable that overrides the source
// openclaw home directory when migrating from openclaw to autoforze.
// Default: ~/.openclaw
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	picoCfg, warnings, err := openclawCfg.ConvertToAutoForze(o.opts.SourceHome)
	if err != nil {
		return err
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".autoforze", 1)
	return path
}
