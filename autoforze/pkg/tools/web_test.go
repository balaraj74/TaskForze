...
// with cf-mitigated: challenge triggers a retry using the honest autoforze User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest autoforze user agent
	if !strings.Contains(receivedUAs[1], "autoforze") {
		t.Errorf("retry request UA = %q, want it to contain 'autoforze'", receivedUAs[1])
	}
}
