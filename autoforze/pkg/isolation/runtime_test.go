...
func TestResolveInstanceRoot_UsesAutoforzeHome(t *testing.T) {
	t.Setenv(config.EnvHome, "/custom/autoforze/home")
	root, err := ResolveInstanceRoot()
	if err != nil {
		t.Fatalf("ResolveInstanceRoot() error = %v", err)
	}
	if root != "/custom/autoforze/home" {
		t.Fatalf("ResolveInstanceRoot() = %q, want %q", root, "/custom/autoforze/home")
	}
}
...
func TestBuildWindowsAccessRules(t *testing.T) {
	...
	for _, rule := range rules {
		if rule.Path == `C:\autoforze` && rule.Mode == "rw" {
			foundRoot = true
		}
