...
`pkg/isolation` provides process-level isolation for child processes started by `autoforze`.

It does not sandbox the main `autoforze` process itself.
...
- The `autoforze` main process still runs in the host environment.
  ...
  "isolation": {
    ...
    "expose_paths": [
      ...
      },
      {
        ...
        "target": "/opt/autoforze-instance-a/workspace/assets",
        "mode": "rw"
      }
    ]
  }
...
- If `AUTOFORZE_HOME` is set, use it.
- Otherwise use the default `.autoforze` directory under the user home.
...
At runtime, AutoForze also adds the executable path, its directory, the effective working directory, and absolute path arguments when needed.
...
When isolation is enabled, AutoForze logs the generated isolation plan.
...
- The current design isolates child processes, not the main `autoforze` process.
