...
This package handles speech synthesis for AutoForze.
...
| --- | --- |
| [OpenAI](https://platform.openai.com/docs/guides/text-to-speech) | Best-supported path in AutoForze today. The current TTS implementation is built around the OpenAI-compatible `/audio/speech` API shape, and OpenAI is the safest default. |
...
AutoForze does not keep TTS API keys inside `voice`.
...
If you use a custom MiMo endpoint, you can also set `api_base` explicitly. Otherwise AutoForze will use the provider default.

## What AutoForze Sends Today
...
- AutoForze currently does not expose a user-facing config field for changing the TTS voice from `alloy`.

## How AutoForze Chooses a TTS Provider

`DetectTTS` resolves TTS in this order:

1. **Preferred path**: resolve `voice.tts_model_name` against `model_list`.
2. If a matching model entry exists and has an API key, AutoForze creates an OpenAI-compatible TTS provider using that model's settings.
3. **Fallback path**: if `voice.tts_model_name` is not set or cannot be resolved, AutoForze scans `model_list` for the first entry whose model string contains `tts` and has an API key.
...
AutoForze normalizes the configured base URL for TTS:

- For OpenAI, a base like `https://api.openai.com` or `https://api.openai.com/v1` becomes `https://api.openai.com/v1/audio/speech`.
- For other OpenAI-compatible providers, AutoForze preserves the configured base path and ensures it ends with `/audio/speech`.
- If `api_base` is omitted, AutoForze uses the provider default base when the model prefix is known.
...
- Assuming AutoForze will automatically use provider-specific custom voices.
