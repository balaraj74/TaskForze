...
This package handles speech-to-text for AutoForze voice input.
...
AutoForze does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of AutoForze's model configuration.
...
- You can omit `api_base` and AutoForze will use Groq's default API base automatically.
...
AutoForze currently supports three main ASR routes:
...
## How AutoForze Chooses a Transcriber
...
2. If that resolved model is:
   - `elevenlabs/...`, AutoForze uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, AutoForze uses the Whisper transcriber.
   - an audio-capable chat model, AutoForze uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, AutoForze performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.
