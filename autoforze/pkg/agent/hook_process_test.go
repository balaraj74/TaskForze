...
func TestAgentLoop_MountProcessHook_IsolationSupportsRelativeDirAndCommand(t *testing.T) {
	...
	root := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(root, "autoforze-home"))
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"AUTOFORZE_HOOK_HELPER=1",
		"AUTOFORZE_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "AUTOFORZE_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("AUTOFORZE_HOOK_MODE")
	eventLog := os.Getenv("AUTOFORZE_HOOK_EVENT_LOG")
