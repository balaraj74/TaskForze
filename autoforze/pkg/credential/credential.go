...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via AUTOFORZE_KEY_PASSPHRASE
...
//  2. AUTOFORZE_SSH_KEY_PATH env var
//  3. ~/.ssh/autoforze_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "AUTOFORZE_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading AUTOFORZE_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "AUTOFORZE_SSH_KEY_PATH"

// autoforzeHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const autoforzeHome = "AUTOFORZE_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "autoforze-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (AUTOFORZE_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// AUTOFORZE_SSH_KEY_PATH env var or ~/.ssh/autoforze_ed25519.key.
...
//   - exact match with AUTOFORZE_SSH_KEY_PATH env var
//   - within the AUTOFORZE_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with AUTOFORZE_SSH_KEY_PATH.
	...
	// Within AUTOFORZE_HOME.
	if picoHome := os.Getenv(autoforzeHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="autoforze-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set AUTOFORZE_SSH_KEY_PATH or place key at ~/.ssh/autoforze_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (AUTOFORZE_SSH_KEY_PATH, AUTOFORZE_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. AUTOFORZE_SSH_KEY_PATH env var
//  3. ~/.ssh/autoforze_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the autoforze-specific SSH key path if it exists.
