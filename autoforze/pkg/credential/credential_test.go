...
func TestResolve_EncKey_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	sshKeyPath := filepath.Join(dir, "autoforze_ed25519.key")
	...
	t.Setenv("AUTOFORZE_SSH_KEY_PATH", sshKeyPath)
	...
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", passphrase)
...
func TestResolve_EncKey_WithSSHKey(t *testing.T) {
	...
	// Set AUTOFORZE_SSH_KEY_PATH before Encrypt so the path passes allowedSSHKeyPath validation.
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", passphrase)
	t.Setenv("AUTOFORZE_SSH_KEY_PATH", sshKeyPath)
...
func TestResolve_EncKey_NoPassphrase(t *testing.T) {
	...
	_, err = r.Resolve(enc)
	if err == nil {
		t.Fatal("expected error when AUTOFORZE_KEY_PASSPHRASE is unset, got nil")
	}
}

func TestResolve_EncKey_BadCiphertext(t *testing.T) {
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", "some-passphrase")
...
func TestResolve_EncKey_WrongPassphrase(t *testing.T) {
	...
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", "wrong-passphrase")
...
func TestDeriveKey_SSHKeyNotFound(t *testing.T) {
	...
	t.Setenv("AUTOFORZE_KEY_PASSPHRASE", "passphrase")
	t.Setenv("AUTOFORZE_SSH_KEY_PATH", filepath.Join(dir, "nonexistent_key"))
...
// that are not under AUTOFORZE_SSH_KEY_PATH, AUTOFORZE_HOME, or ~/.ssh/.
