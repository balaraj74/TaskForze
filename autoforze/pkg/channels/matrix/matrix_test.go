...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("autoforze")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@autoforze hello", want: true},
		{text: "hi @autoforze:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bautoforze\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@autoforze:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@autoforze:matrix.org hello", want: "hello"},
		{in: "@autoforze, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@autoforze:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@autoforze:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@autoforze:matrix.org">AutoForze</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40autoforze%3Amatrix.org">AutoForze</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@autoforze:matrix.test"), "")
	if err != nil {
