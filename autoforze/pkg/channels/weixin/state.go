...
import (
	...
	basechannels "github.com/sipeed/autoforze/pkg/channels"
...
func buildWeixinSyncBufPath(cfg config.WeixinConfig) string {
	return filepath.Join(autoforzeHomeDir(), "channels", "weixin", "sync", genWeixinAccountKey(cfg)+".json")
}

func buildWeixinContextTokensPath(cfg config.WeixinConfig) string {
	return filepath.Join(autoforzeHomeDir(), "channels", "weixin", "context-tokens", genWeixinAccountKey(cfg)+".json")
}
