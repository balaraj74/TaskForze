...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	// Extract
	destDir, err := os.MkdirTemp("", "autoforze-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
// Example: owner="sky5454" -> https://api.github.com/repos/sky5454/autoforze/releases/latest
func GetReleaseAPIURL(owner string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/autoforze/releases/latest", owner)
}
...
// Example: owner="sipeed", tag="nightly" -> https://api.github.com/repos/sipeed/autoforze/releases/tags/nightly
func GetReleaseTagAPIURL(owner, tag string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/autoforze/releases/tags/%s", owner, tag)
}
